...
def create_optimizer(model):
    return torch.optim.Adam(params=list(model.parameters()), lr=1e-4, betas=(0.9, 0.999), weight_decay=1e-4)

def arguments():
    parser = argparse.ArgumentParser(description='AVIN-TF Training')
...
def main_worker(gpu, args):
    ...
    if gpu == 0:
        ...
        logger = utils.get_root_logger(log_file, 'AVIN-TF')
        args_dict = vars(args)
